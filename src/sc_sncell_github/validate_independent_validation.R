...
for (variant in c("baseline","donor_balanced","epithelial_split","balanced_split")) {
  inp <- file.path(root,"data/processed/Wu2021_independent_validation_v1",variant)
  out <- file.path(root,"results/Wu2021_independent_validation_v1",variant)
  ...
  checks[[paste0(variant,"_rds_csv")]] <- isTRUE(all.equal(actual,raw,tolerance=1e-12))
  ...
  checks[[paste0(variant,"_settings")]] <- record$seed==9 && record$mode=="full" && record$max_cores==2
}
capture.output(checks,file=file.path(root,"results/Wu2021_independent_validation_v1/rds_validation.txt"))
print(checks)
